...
@section('breadcrumb')
    <div class="mr-auto">
        ...
        <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
            ...
            <li class="m-nav__item">
                <a href="{{ route('admin.cities.edit', ['id' => $info->id]) }}" class="m-nav__link m--font-brand">
                    <span class="m-nav__link-text">
                        {{ $info->name }}
                    </span>
                </a>
...
@section('content')
    ...
    <div class="m-portlet">
        ...
        <!--begin::Form-->
        <form class="m-form m-form--fit m-form--label-align-right" id="frmEditCity" action="{{ route('admin.cities.edit', ['id' => $info->id_hash]) }}" method="post">
                    ...
                    <div class="col-3">
                        <select class="form-control m-bootstrap-select m_selectpicker" data-live-search="true" name="country_id">
                            ...
                            @foreach($countries as $row)
                                <option value="{{ $row->id }}" {{ $row->id == $info->country_id ? 'selected' : '' }}>{{ $row->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
